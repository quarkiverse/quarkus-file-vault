...
== Masking the secret keystore

If you need to masking the `quarkus.file.vault.provider.<name>.secret` you will need to build and package the link:https://github.com/quarkiverse/quarkus-file-vault/tree/main/vault-utils[Vault Utils] project.
...
Usage: Encrypt Secret Util [-hV] -e=<encryptionKey> [-i=<iterationCount>] -p=<keystorePassword> [-s=<salt>]
...
The two only mandatory parameters are the keystore password: `-p, --keystore-password`, and the encryption key: `-e, --encryption-key`.

You can create the mask for example:

[source,shell]
----
$ java -jar target/quarkus-app/quarkus-run.jar -p storedpass -e justsomestringhere -i 506235 -s 12344321
----
...
quarkus.file.vault.provider.<name>.salt=12344321
quarkus.file.vault.provider.<name>.encryption-key=justsomestringhere
quarkus.file.vault.provider.<name>.iteration-count=506235
